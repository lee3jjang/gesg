...
//	private static int flushSize = 50;
	...
	private static String historyDataTerm		= "WEEK"; 
	private static String historyDataFromYn		=	"Y";
	private static int	 	historyDataMonth	=	-120;
	private static String 	historyDataFrom		=	"201201";
	private static String shockModel			=	"AFNS";
	private static String shockCurveId			=	"A100";
	private static String shockStBssd ;
...
// ******************************************************************ESG Simulation Job ********************************
		...
		job190();		// job19 : AFNS 충격 시나리오 생성 TODO : 테이블 생성 & 저장 로직
		job19();		// job19 : AFNS 충격 시나리오 생성 TODO : 테이블 생성 & 저장 로직
...
//		kicsVolAdjust     = Double.parseDouble(argumentMap.getOrDefault("kicsVolAdjust", properties.getOrDefault("kicsVolAdjust", "0.0032").toString()));
		...
		historyDataMonth	=	Integer.parseInt(argumentMap.getOrDefault("historyDataMonth", properties.getOrDefault("historyDataMonth", "-120").toString()));
		
		shockModel 			=	argumentMap.getOrDefault("shockModel", properties.getOrDefault("shockModel", "AFNS").toString());
		shockCurveId		=	argumentMap.getOrDefault("shockCurveId", properties.getOrDefault("shockCurveId", "A100").toString());
		
		historyDataTerm		= 	argumentMap.getOrDefault("historyDataTerm", 	properties.getOrDefault("historyDataTerm", "WEEK").toString()); 
		historyDataFrom		=	argumentMap.getOrDefault("historyDataFrom", 	properties.getOrDefault("historyDataFrom", "201201").toString());
		historyDataFromYn	=	argumentMap.getOrDefault("historyDataFromYn", 	properties.getOrDefault("historyDataFromYn", "Y").toString());
		
		if(historyDataFromYn.equals("Y")) {
			shockStBssd = historyDataFrom;
		}
		else {
			shockStBssd = FinUtils.addMonth(bssd, historyDataMonth);
		}
				
		afnsTenorListStr    = properties.getOrDefault("AFNS_TENOR_LIST", "M0003,M0006,M0009,M0012,M0018,M0024,M0030,M0036,M0048,M0060,M0084,M0120,M0240").toString();
		...
		for(int i=0; i<tempString.length; i++) {
			kicsVolAdjustMap.put(tempString[i].trim(), Double.valueOf(tempValue[i].trim()));
		}
...
//				금리 시나리오 파일 결과 Write
	...
	private static void job15DBOnly() {
		if (jobList.contains("15")) {
			...
			try {
				...
				for(IrCurve aa : rfCurveList) {	
					logger.info("Ir Scenario for {} : {}", aa.getCurCd(), aa.getIrCurveId());
	...
	private static void job190() {
		if (jobList.contains("190")) {
			LocalDateTime startTime = LocalDateTime.now();                              
			LogTable logTable = getLogTable("BADA1001JM", startTime,bssd);    

			logger.info("Job 19(AFNS Shock Scenario) START at {}!!!", startTime);
			session.beginTransaction();
			try {
				String stBssd =  shockStBssd;
				String irCurveId = shockCurveId;
				
				int deleteNum = session.createQuery("delete IrCurveWeek a where a.irCurveId = :param and a.baseDate >= :param1 and a.baseDate <= :param2")
								.setParameter("param", irCurveId)
								.setParameter("param1", stBssd)
								.setParameter("param2", bssd+"31").executeUpdate();
				
				logger.info("Job 19(AFNS Setup Week IR Curve) delete {} rows in the db", deleteNum);

				Job19_AfnsScenarioSetup.setupIrCurveWeek(bssd, stBssd, irCurveId, afnsTenorList, DayOfWeek.FRIDAY).forEach(s-> session.save(s));
				
//				List<IrCurveWeek> rstList = Job19_AfnsScenarioSetup.setupIrCurveWeek(bssd, stBssd, irCurveId, afnsTenorList, DayOfWeek.FRIDAY);
			...
			session.saveOrUpdate(logTable); 
			session.getTransaction().commit();
			logger.info("Job 19(AFNS Shcok Scenario) is Completed !!!");
...
//				String stBssd 			= "201601";
				String irCurveId 		= shockCurveId;
				String baseCurveId     	= shockCurveId;
				String mode            	= shockModel;
				double ltfrL           	= swParamMap.get("KRW").getUfr();
				double ltfrA           	= swParamMap.get("KRW").getUfr();
				int    ltfrT           	= (int)swParamMap.get("KRW").getUfrT();	
				double liqPrem         	= kicsVolAdjustMap.get("KRW");	
				...
				logger.info("input shock sce : {},{},{}", stBssd, irCurveId, mode);
				Map<String, List<?>> irShockSenario = new TreeMap<String, List<?>>();
				
				
				irShockSenario = Job19_AfnsScenario.createAfnsShockScenario(FinUtils.toEndOfMonth(bssd), mode, irCurveId, baseCurveId, afnsTenorList, 
...
//					BIZ APPLY DCNT  : M0001~M1200 
					...
					String irCurveId = aa.getIrCurveId();
...
//				Job25_KicsTermStructure.createKicsTermStructure(bssd, kicsList, kicsVolAdjustMap).stream().forEach(s -> session.saveOrUpdate(s));
				Job25_KicsTermStructure.createKicsTermStructureNew(bssd, kicsList, irCurveTenor, kicsVolAdjustMap).stream().forEach(s -> session.saveOrUpdate(s));
...
//				Job251_KicsBizApplyDcnt.getBizDcntRateKics(bssd, kicsList).stream().forEach(s -> logger.info("aaa  : {},{}", s.toString()));
...
//				Job63_IFRSBizApplyRC.getBizInflationMA(bssd).stream().forEach(s ->session.saveOrUpdate(s));
				...
				infRst.forEach(s -> session.saveOrUpdate(s));
