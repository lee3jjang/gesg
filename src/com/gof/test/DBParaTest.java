...
//		ddd();
	...
	private static void bbb() {
		...
		List<IrCurveHis> irCurveHis = IrCurveHisDao.getIrCurveHis("201712");
		List<IrCurveHis> irCurveHis1 = irCurveHis.stream().filter(s -> s.getMatCd().startsWith("M001")).collect(Collectors.toList());
		
		
		for(IrCurveHis aa : irCurveHis1) {
			fuList.add(exe.submit(new FilePara3(aa)));
		}
