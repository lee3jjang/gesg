...
import com.gof.entity.HisDiscRateStat;
...
//					   ;
	...
	public static InvestManageCostUd getCurrentUserInvMgtCost(String bssd){
		String maxBssdquery = " SELECT MAX(BASE_YYMM) "  
				+ "				from ESG.EAS_USER_INV_MGT_COST   a "
				+ "				where 1=1 "
	...
	/*public static BigDecimal  getMaxSeq(String bssd, String bizDiv){
		
		String query = "SELECT MAX(SEQ) "  
		+ "				from EAS_HIS_DISC_RATE_STAT a "
		+ "				where 1=1 "
		+ "				and a.BASE_YYMM = :bssd	"
		+ "				and a.APPL_BIZ_DV   = :bizDiv	"
		;
		
		Object maxSeq =  session.createNativeQuery(query)
				.setParameter("bssd", bssd)
...
//			return 0.0;
	...
	public static int  getMaxSeq(String bssd, String bizDiv){
		...
		String query = "select  a from HisDiscRateStat a "
		+ "				where 1=1 "
		+ "				and a.baseYymm = :bssd	"
		+ "				and a.applyBizDv   = :bizDiv"
		;
		
		return session.createQuery(query,  HisDiscRateStat.class)
		   .setParameter("bssd",bssd)
		   .setParameter("bizDiv",bizDiv)
